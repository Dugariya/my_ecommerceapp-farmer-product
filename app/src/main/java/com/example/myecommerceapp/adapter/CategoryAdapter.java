...
public class CategoryAdapter extends RecyclerView.Adapter<CategoryAdapter.VieHolder> {
    ...
    public VieHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new VieHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.category_list,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull VieHolder holder, int position) {

        Glide.with(context).load(list.get(position).getImg_url()).into(holder.catImag);
    ...
    public class VieHolder extends RecyclerView.ViewHolder {
        ...
        public VieHolder(@NonNull View itemView) {
            super(itemView);
             catImag = itemView.findViewById(R.id.cat_img);
